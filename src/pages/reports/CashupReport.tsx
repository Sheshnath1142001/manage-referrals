...
interface CashupData {
  ...
  note: string | null;
  status: number;
  additional_payments: Record<string, AdditionalPayment>;
}
...
const CashupReport = () => {
  ...
  const fetchCashupData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching cashup data:", error);
      setCashupData(null);
      toast({
        title: "Error",
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold">Cashup Report</h1>
        </div>
          ...
          <Table>
            <TableHeader className="bg-[#0F172A]">
              <TableRow>
                ...
                <TableHead className="text-white font-medium text-right">Other amount</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {!cashupData?.cashups.length ? (
                ...
                cashupData.cashups.map((cashup) => (
                  <TableRow key={cashup.id}>
                    ...
                    <TableCell className="text-right">{formatCurrency(cashup.other_amount)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
